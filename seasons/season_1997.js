d3.json("kobe_bryant.json").then(data => {
    const seasonData = data.find(d => d.season === 1997);
    if (!seasonData) {
        console.error("1997 season not found.");
        return;
    }
    ...
    //color pairs for normal and hover states
    const colorPairs = {
        "ft": { normal: "gold", hover: "#ffffa0" },       
        "two": { normal: "orange", hover: "#ffcc80" },      
        "three": { normal: "crimson", hover: "#ff6b6b" }    
    };
    ...
    svg.append("text")
        ...
        .text("Kobe Bryant - 1997 Scoring Zones");
    svg.append("text")
        .attr("x", width / 2)
        .attr("y", 60)
        .attr("text-anchor", "middle")
